...
static void check_init_error_on_object_is_null(void **state) {
    ...
    assert_false(rock_tree_set_init(NULL, 1, (void *)1));
...
static void check_init_error_on_size_is_zero(void **state) {
    rock_error = ROCK_ERROR_NONE;
    assert_false(rock_tree_set_init((void *)1, 0, (void *)1));
    assert_int_equal(ROCK_TREE_SET_ERROR_SIZE_IS_ZERO, rock_error);
    rock_error = ROCK_ERROR_NONE;
}

static void check_init_error_on_compare_is_null(void **state) {
    rock_error = ROCK_ERROR_NONE;
    assert_false(rock_tree_set_init((void *)1, 1, NULL));
...
static void check_size_error_on_object_is_null(void **state) {
    rock_error = ROCK_ERROR_NONE;
    assert_false(rock_tree_set_size(NULL, (void *)1));
    assert_int_equal(ROCK_TREE_SET_ERROR_OBJECT_IS_NULL, rock_error);
    rock_error = ROCK_ERROR_NONE;
}
...
static void check_size_error_on_out_is_null(void **state) {
    rock_error = ROCK_ERROR_NONE;
    assert_false(rock_tree_set_size((void *)1, NULL));
...
static void check_size(void **state) {
    ...
    uintmax_t size;
    assert_true(rock_tree_set_count(&object, &size));
    assert_int_equal(object.size, sizeof(void *));
    rock_error = ROCK_ERROR_NONE;
}
...
static void check_add(void **state) {
    ...
    assert_true(rock_tree_set_add(&object, (void*)&value));
...
static void check_add_error_on_item_already_exists(void **state) {
    ...
    assert_int_equal(1, object.tree.count);
    assert_non_null(object.tree.root);
    assert_false(rock_tree_set_add(&object, (void *)&value));
...
static void check_remove_error_on_item_not_found(void **state) {
    ...
    assert_true(rock_tree_set_init(&object,
                                   sizeof(void *),
                                   seagrass_void_ptr_compare));
    assert_false(rock_tree_set_remove(&object, (void *)&value));
...
static void check_remove(void **state) {
    ...
    assert_true(rock_tree_set_add(&object, (void *)&value));
    assert_int_equal(1, object.tree.count);
    assert_true(rock_tree_set_remove(&object, (void *)&value));
    assert_int_equal(0, object.tree.count);
    assert_true(rock_tree_set_invalidate(&object, NULL));
    rock_error = ROCK_ERROR_NONE;
}
...
static void check_contains_error_item_is_null(void **state) {
    rock_error = ROCK_ERROR_NONE;
...
static void check_contains(void **state) {
    ...
    assert_true(rock_tree_set_init(&object,
                                   ...
                                   seagrass_void_ptr_compare));
    bool result;
    assert_true(rock_tree_set_contains(&object, (void *)&value, &result));
    assert_false(result);
    assert_true(rock_tree_set_add(&object, (void *)&value));
    assert_true(rock_tree_set_contains(&object, (void *)&value, &result));
    assert_true(result);
...
static void check_first_error_on_empty_tree_set(void **state) {
    rock_error = ROCK_ERROR_NONE;
    struct rock_tree_set object = {};
    assert_true(rock_tree_set_init(
            &object,
            sizeof(uintmax_t),
            (int (*)(const void *, const void *))seagrass_uintmax_t_compare));
    uintmax_t first;
...
int main(int argc, char *argv[]) {
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(check_contains_error_item_is_null),
